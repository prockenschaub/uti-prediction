...
n <- 100
x1 <- rnorm(n)
x2 <- rnorm(n, x1, sd = 0.5)
x3 <- abs(x2) + rnorm(n, sd = 0.5)
...
data[sample(c(FALSE, TRUE), n, TRUE, c(0.8, 0.2)), c("x2", "x3")] <- NA


imp1 <- mice(data)
xyplot(imp1, x3 ~ x2 | .imp, pch = c(1, 20), cex = c(1, 1.5))
densityplot(imp1)

imp2 <- mice(data, method = c(x2_x3 = "mvpmm"), formulas = list(x2_x3 = cbind(x2, x3) ~ x1))
xyplot(imp2, x3 ~ x2 | .imp, pch = c(1, 20), cex = c(1, 1.5))
densityplot(imp2)
